  ...
  public:
    ...
    [[nodiscard]] auto getLockAndNotifyWhenDone() const
    {
        struct UnLockWithNotify final
        {
            ...
            {}
            // Mutex interface
            inline void lock() { monitor_.lock_.lock();}
            inline void unlock() 
            {
                if constexpr (broadcast) monitor_.condition_.notify_all();
                else monitor_.condition_.notify_one();
    ...
    /**
     * Wait infinite on the predicate to be signaled, and return 
     * afterwards the locking object to the client
    */
    ...
    */
        ...
        Args&&... args) const
    {
        ...
        const bool result = condition_.wait_for(
            lock,
            timeout,
            [ p = std::forward<Predicate>(predicate), ...args = std::forward<Args>(args)]() mutable
            {
  ...
  private:
      ...
      */
      ...
      auto notify(Func&& func, Args&&...args) const
      {
        ...
        std::lock_guard lock {lockWithNotify};
